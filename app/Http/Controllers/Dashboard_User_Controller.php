...
class Dashboard_User_Controller extends Controller
{
    ...
    public function tambah_slot_microsite($id) {
  	...
  	public function tambah_slot_shortlink($id) {
        $paket = paketModel::find($id);
        $secret_key = 'Basic '.config('xendit.key_auth');
        $external_id = Str::random(10);
        $data_request = Http::withHeaders([
        ...
        $transaksi->id_user = Auth::user()->id;
        $transaksi->external_id = $external_id;
        $transaksi->type = "shortlink";
        $transaksi->total = $paket->harga;
        $transaksi->ref_id = $paket->id; 
        $transaksi->invoice_id = $response->id;
        $transaksi->slot = $paket->slot;
        $transaksi->shortlink = $paket->shortlink;
        $transaksi->url = $response->invoice_url;
        $transaksi->status = $response->status;
        $transaksi->save();
        $user_id->save();
    ...
    public function shortlinks() {
        ...
        if ($user) {
			...
			$tautan = "tidak";
            $shortlink_check = transaksi::where('status', 'PAID')->whereNotNull('shortlink')->where('id_user', Auth::user()->id)->where('flag', 0)->latest()->first();
            $user = User::find(Auth::user()->id);
            if($shortlink_check){
                $user->batas_shortlink = $shortlink_check->shortlink;
                $user->save();
                $shortlink_check->flag = 2;
                $shortlink_check->save();
            }
            
            $berlangganan=(date('Y-m-d') < $user->batas_shortlink);
            if ($berlangganan){
