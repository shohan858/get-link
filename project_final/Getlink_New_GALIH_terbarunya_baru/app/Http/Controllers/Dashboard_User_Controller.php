...
class Dashboard_User_Controller extends Controller
{
    ...
    {
        $visitor = visitortotalModel::all();
        $totalVisitors = [];
        $data_tahun_ini = array();
        $data_tahun_kemarin = array();
        $data_tahun_kemarin2 = array();
        
        foreach ($visitor as $v) {
            $totalVisitors[] = $v->total_visitor;
        }


        foreach ($totalVisitors as $key => $value) {
            if (strpos($value, date('Y')) !== false) {
                $data[$key] = str_replace(',' . date('Y'), '', $value);
                $data_tahun_ini[] = $data[$key];
            } elseif (strpos($value, date('Y', strtotime("-1 year"))) !== false) {
                $data[$key] = str_replace(',' . date('Y', strtotime("-1 year")), '', $value);
                $data_tahun_kemarin[] = $data[$key];
            } elseif (strpos($value, date('Y', strtotime("-2 year"))) !== false) {
                $data[$key] = str_replace(',' . date('Y', strtotime("-2 year")), '', $value);
                $data_tahun_kemarin2[] = $data[$key];
            }
        }
